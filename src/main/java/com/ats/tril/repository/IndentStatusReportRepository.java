...
public interface IndentStatusReportRepository extends JpaRepository<IndentStatusReport, Integer>{
	...
	@Query(value = "select @a\\:=@a+1 sr,id.ind_d_id,i.ind_m_no,i.ind_m_date,id.item_id,concat(m.item_code,' ',m.item_desc) as item_code,"
			+ "id.ind_item_schddt, coalesce(' ') as remark ,DATEDIFF(:fromDate,id.ind_item_schddt) AS excess_days from indent i,"
			+ "indtrans id,m_item m , (SELECT  @a\\:=:index) AS a where id.ind_m_id=i.ind_m_id and m.item_id=id.item_id and "
			+ "i.del_status=1 and id.del_status=1 and id.ind_d_status in (0) and i.ind_m_date "
	...
	@Query(value = "select  @a\\:=@a+1 sr,id.ind_d_id,i.ind_m_no,i.ind_m_date,id.item_id,concat(m.item_code,' ',m.item_desc) as item_code,"
			+ "id.ind_item_schddt, coalesce(ph.po_no) as remark,DATEDIFF(:fromDate,id.ind_item_schddt) AS excess_days from indent i,"
	...
	@Query(value = "select @a\\:=@a+1 sr,id.ind_d_id,i.ind_m_no,i.ind_m_date,id.item_id,concat(m.item_code,' ',m.item_desc) as item_code,"
			+ "id.ind_item_schddt, coalesce(mh.mrn_no) as remark,DATEDIFF(:fromDate,id.ind_item_schddt) AS excess_days from indent i,indtrans id,"
