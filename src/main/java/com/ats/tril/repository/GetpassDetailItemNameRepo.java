...
public interface GetpassDetailItemNameRepo extends JpaRepository<GetpassDetailItemName, Integer> {
	...
	@Query(value = "SELECT h.*,CONCAT(i.item_code, '-', i.item_desc) as item_code, c.cat_id, g.grp_id FROM t_gatepass_detail h , m_item i, m_category c, m_item_group g "
	...
	@Query(value = "SELECT h.*,CONCAT(i.item_code, '-', i.item_desc) as item_code, c.cat_id, g.grp_id FROM t_gatepass_detail h , m_item i, m_category c, m_item_group g "
			+ "WHERE i.item_id=h.gp_item_id AND gp_id=:gpId and h.is_used=1 and c.cat_id=i.cat_id and g.grp_id = i.grp_id ", nativeQuery = true)
