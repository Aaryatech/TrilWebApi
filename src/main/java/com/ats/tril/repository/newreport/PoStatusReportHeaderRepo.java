...
public interface PoStatusReportHeaderRepo extends JpaRepository<PoStatusReportHeader, Integer>  {
	...
	@Query(value = " SELECT m_item.item_code,m_item.item_desc,m_item.item_id," + 
			...
			" AND po_header.po_type=:typeId AND m_item.cat_id=:catId AND po_header.po_status IN (:statusList) "
			+ "  " + 
			"" + 
	...
	List<PoStatusReportHeader> getPoStatusReportHeaderByTypeAndCatId(@Param("fromDate") String fromDate,
			@Param("toDate") String toDate,@Param("typeId") int typeId,@Param("catId") int catId,@Param("statusList") List<String> statusList);
	...
	@Query(value = " SELECT m_item.item_code,m_item.item_desc,m_item.item_id," + 
			...
			" and indent.ind_m_id=po_header.ind_id " + 
			" AND po_header.po_date BETWEEN :fromDate AND :toDate " + 
			" and m_item.cat_id=:catId  AND po_header.po_status IN (:statusList)" + 
			"  " + 
			" ORDER BY m_item.item_code", 
			nativeQuery = true)
	List<PoStatusReportHeader> getPoStatusReportHeaderAllTypeOnecatId(@Param("fromDate") String fromDate, @Param("toDate") String toDate, @Param("catId") int catId,@Param("statusList") List<String> statusList);
	...
	@Query(value = " SELECT m_item.item_code,m_item.item_desc,m_item.item_id," + 
			...
			" AND po_header.vend_id=m_vendor.vendor_id " + 
			" and indent.ind_m_id=po_header.ind_id " + 
			" AND po_header.po_date BETWEEN :fromDate AND :toDate AND po_header.po_status IN (:statusList) " + 
			" ORDER BY m_item.item_code", 
			nativeQuery = true)
	List<PoStatusReportHeader> getPoStatusReportHeaderAllTypeAllcatId(@Param("fromDate") String fromDate,
			@Param("toDate") String toDate,@Param("statusList") List<String> statusList);
// all cate one type -4
	...
	@Query(value = " SELECT m_item.item_code,m_item.item_desc,m_item.item_id," + 
			...
			" and indent.ind_m_id=po_header.ind_id " + 
			" AND po_header.po_date BETWEEN :fromDate AND :toDate " + 
			" AND po_header.po_type=:typeId AND po_header.po_status IN (:statusList)  "
			+ "  " + 
			"" + 
			"  ORDER BY m_item.item_code ", 
			nativeQuery = true)
	...
	List<PoStatusReportHeader> getPoStatusReportHeaderByTypeAndAllCatId(@Param("fromDate") String fromDate,
			@Param("toDate") String toDate,@Param("typeId") int typeId,@Param("statusList") List<String> statusList);
