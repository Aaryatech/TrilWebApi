...
public interface StockValuationCategoryWiseRepository extends JpaRepository<StockValuationCategoryWise, Integer>{
	...
	@Query(value=(" SELECT m_category.cat_id, m_category.cat_desc, coalesce(0) AS opening_stock, coalesce(0) AS op_stock_value, "
			...
			+ "and t_mrn_header.del_status=1 and t_mrn_detail.del_status=1 and m_item.cat_id=m_category.cat_id and t_mrn_header.mrn_type=:typeId),0) AS approve_qty, "
			...
			+ "and t_mrn_detail.del_status=1 and po_detail.po_detail_id=t_mrn_detail.po_detail_id and m_item.cat_id=m_category.cat_id and t_mrn_header.mrn_type=:typeId), 0) AS approved_qty_value, "
			...
			+ "and t_mrn_header.mrn_type=:typeId), 0) AS approved_landing_value, coalesce((Select SUM(item_issue_detail.item_issue_qty) FROM item_issue_header, "
			...
			+ "and item_issue_header.item_category=:typeId), 0) AS issue_qty, coalesce((Select SUM(item_issue_detail.item_issue_qty*po_detail.item_rate) "
			...
			+ "and po_detail.po_detail_id=t_mrn_detail.po_detail_id and m_item.cat_id=m_category.cat_id and item_issue_header.item_category=:typeId), 0) AS issue_qty_value, "
			...
			+ "and item_issue_detail.mrn_detail_id=t_mrn_detail.mrn_detail_id and po_detail.po_detail_id=t_mrn_detail.po_detail_id and m_item.cat_id=m_category.cat_id and item_issue_header.item_category=:typeId), 0) AS issue_landing_value, "
