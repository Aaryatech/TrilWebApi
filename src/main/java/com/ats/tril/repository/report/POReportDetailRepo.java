...
public interface POReportDetailRepo extends JpaRepository<POReportDetail, Integer> {
	@Query(value = "SELECT *,i.item_code,id.ind_m_date FROM po_detail ,m_item i ,indent id WHERE po_detail.po_id IN(:poIdList)  AND i.item_id=po_detail.item_id AND po_detail.ind_id=id.ind_m_id ", nativeQuery = true)
