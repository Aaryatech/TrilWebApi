...
public interface RejectionReportDetailRepo extends JpaRepository<RejectionReportDetail, Integer> {
	...
	@Query(value = "SELECT d.*,i.item_code,i.item_desc FROM t_rejection_memo_detail d,m_item i WHERE  d.rejection_id IN(:rejectionIdList) AND i.item_id=d.item_id AND d.is_used=1", nativeQuery = true)
