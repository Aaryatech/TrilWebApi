...
public interface GatepassReportRepository extends JpaRepository<GatepassReport, Integer> {
	...
	@Query(value = "SELECT h.*,v.vendor_name,v.vendor_code ,v.vendor_add1 FROM t_gatepass_header h ,m_vendor v WHERE  h.gp_type=1 AND h.gp_status !=3  AND h.gp_vendor=v.vendor_id AND h.is_used=1 AND h.gp_id IN(:gpIdList)"
			+ "", nativeQuery = true)
