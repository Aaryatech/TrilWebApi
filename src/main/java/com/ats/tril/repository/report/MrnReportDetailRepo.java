...
public interface MrnReportDetailRepo extends JpaRepository<MrnReportDetail, Integer> {
	...
	@Query(value = "SELECT d.*,i.item_desc,i.item_code FROM  t_mrn_detail d ,m_item i WHERE d.del_status=1 AND d.mrn_id IN(:mrnIdList)  AND i.item_id=d.item_id   ", nativeQuery = true)
