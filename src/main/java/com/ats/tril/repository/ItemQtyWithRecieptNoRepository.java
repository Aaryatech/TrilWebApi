...
public interface ItemQtyWithRecieptNoRepository extends JpaRepository<ItemQtyWithRecieptNo, Integer>{
	...
	@Query(value="Select item_issue_detail.issue_detail_id as id, item_issue_header.issue_date as date, "
			+ "item_issue_header.issue_no as recept_no, item_issue_detail.item_issue_qty as qty FROM item_issue_detail, item_issue_header "
			+ "where item_issue_header.issue_date =:date AND item_issue_header.issue_id=item_issue_detail.issue_id "
			+ "AND item_issue_detail.item_id=:itemId and item_issue_header.delete_status=1 and item_issue_detail.del_status=1 ",nativeQuery=true)
	List<ItemQtyWithRecieptNo> getIssueDataByDate(@Param("date") String date,@Param("itemId") int itemId);
